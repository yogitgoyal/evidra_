...
import { copilotSeed, storyClaims } from "@/lib/data";
...
// Canned allowlisted query responses
const canned: Record<string, CopilotMessage> = {
  "Why is account ••4471 flagged?": copilotSeed[1],
  "Show the shortest path between Subject A and K. Sethi": {
    id: "r1",
    role: "evidra",
    text: "Shortest path: Unknown Subject A → R. Malhotra → K. Sethi, via two observed CALLED edges — 2 hops, both high-confidence.",
    claims: [storyClaims[2]],
    suggestions: ["Who else shares IMEI •••8841?", "Generate the case report"],
  },
  "Who else shares IMEI •••8841?": {
    id: "r2",
    role: "evidra",
    text: "IMEI •••8841 is used by two SIMs — Subject A's registered phone and an unregistered SIM active 3 hours later, treated as the same physical device.",
    claims: [storyClaims[3]],
    suggestions: ["Generate the case report", "Why is account ••4471 flagged?"],
  },
  "Generate the case report": {
    id: "r3",
    role: "evidra",
    text: "I've queued a full report — narrative, graph snapshot, timeline, and evidence appendix. Open the Report Generator to export it as a PDF.",
    suggestions: ["Why is account ••4471 flagged?"],
  },
};
...
export default function CopilotPage() {
  const [messages, setMessages] = useState<CopilotMessage[]>(copilotSeed);
  ...
  function send(text: string) {
    ...
    setTimeout(() => {
      const reply = canned[text] ?? {
        id: "fallback",
        role: "evidra" as const,
        text: "That query isn't in the allowlisted intent catalogue for this demo — try one of the suggestions below, or ask about account ••4471, the shared IMEI, or the shortest path between entities.",
        suggestions: ["Why is account ••4471 flagged?", "Show the shortest path between Subject A and K. Sethi"],
      };
      setMessages((prev) => [...prev, { ...reply, id: nextId("a") }]);
      setThinking(false);
    }, 950);
  }
